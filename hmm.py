...
class StockPredictor(object):
    def __init__(self, company, test_size=0.1,
        ...
        self.hmm = GaussianHMM(n_components=n_hidden_states, n_iter = 10, init_params="mcs", covariance_type = 'diag')
    ...
    def predict_close_price(self, day_index):
        ...
        predicted_frac_change, predicted_high_change, _ = self._get_most_probable_outcome(
            day_index)
        return open_price * (1 + predicted_frac_change)
 
    def predict_close_price_new(self, day_index):
        open_price = self._test_data.iloc[day_index]['open']
        predicted_frac_change, predicted_high_change, _ = self._get_most_probable_outcome(
            day_index)
        high_price = open_price/(1-predicted_high_change)
        return open_price + predicted_frac_change*high_price
    ...
    def predict_close_prices_for_days(self, days, with_plot=False):
        ...
        #We are plotting from row 2266 to row 2365
        if with_plot:
